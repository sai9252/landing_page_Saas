      ...
      <div className=" container">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-center gap-6 items-center mt-10">
          {pricingTiers.map(({ title, monthlyPrice, buttonText, popular, inverse, features }, index) => (
            ...
            <div key={index} className={twMerge("card", inverse === true && ' border-black bg-black text-white/60')}> {/* Ideally, use a unique identifier instead of index */}
              <div className="flex justify-between">
              <h3 className={twMerge("text-lg font-bold text-black/50", inverse === true && 'text-white/60')}>{title}</h3>
              {popular && (

              <div className="inline-flex text-sm px-4 py-1.5 rounded-xl borber border-white/20 ">
              <motion.span
              animate={{
